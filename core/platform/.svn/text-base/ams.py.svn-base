...
class AMS(Agent,Pyro.core.ObjBase):
    ...
    def init_dist(self, dist):
        if dist == Dist.NONE:
            ...
            self.__ams_server = Pyro.core.getProxyForURI('PYRONAME://' + AMS.PYRONAME)
        else:
...
#            self.resume_agent(i.name)
    ...
    def move_agent(self, name, target):
        ...
        self.__remote_amses_lock.acquire()
        if isinstance(target, Pyro.core.DynamicProxy):
            target.receive_agent(agent)
        else:
            Pyro.core.getProxyForURI(target).receive_agent(agent)
