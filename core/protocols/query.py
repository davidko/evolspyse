...
class AwaitInitiationBehaviour(StateBehaviour):
    def action(self):
        if message_key in self.datastore:
            ...
            msg = self.agent.get_message({'performative': ACLMessage.QUERY_REF})
        ...
        elif msg.performative == ACLMessage.QUERY_REF:
            self.datastore[message_key] = msg
...
class QueryRefParticipantBehaviour(FSMBehaviour):
    ...
    def __init__(self, name='', datastore=None, send_response=None, perform_request=None, send_result=None, cancel=None, **namedargs):
        ...
        super(QueryRefParticipantBehaviour, self).__init__(name, **namedargs)
...
class SendQueryRefBehaviour(InitiatorStateBehaviour):
    def __init__(self, name='', content=None, handle_no_participant=None, **namedargs):
        ...
        super(SendQueryRefBehaviour, self).__init__(name, **namedargs)

    def sub_action(self):
        msg = ACLMessage(ACLMessage.QUERY_REF)
...
class QueryInitiatorBehaviour(FSMBehaviour):
    """Behaviour for initiator role."""

    def __init__(self, name='', datastore=None, store=None, request=None, handle_no_participant=None, handle_response=None, handle_agree=None, handle_refuse=None, handle_inform=None, handle_inform_done=None, handle_failure=None, cancel=None, **namedargs):
        if datastore is None:
            datastore = {}
        ...
        super(QueryInitiatorBehaviour, self).__init__(name, **namedargs)
        #
        # States
        #
        send_initiation_state = State(SendQueryRefBehaviour(datastore=datastore, handle_no_participant=self.handle_no_participant, name="Send-initiation"))
        await_response_state = State(AwaitResponseBehaviour(datastore=datastore, name="Await-response"))
        handle_response_state = State(HandleResponseBehaviour(datastore=datastore, handle_response=self.handle_response, name="Handle-response"))
        handled_response_state = State(HandledResponseBehaviour(datastore=datastore, name="Handled-response"))
        handle_agree_state = State(HandleAgreeBehaviour(datastore=datastore, handle_agree=self.handle_agree, name="Handle-agree"))
        handle_refuse_state = State(HandleRefuseBehaviour(datastore=datastore, handle_refuse=self.handle_refuse, name="Handle-refuse"))
        handle_inform_state = State(HandleInformBehaviour(datastore=datastore, handle_inform=self.handle_inform, name="Handle-inform"))
        handle_inform_done_state = State(HandleInformDoneBehaviour(datastore=datastore, handle_inform_done=self.handle_inform_done, name="Handle-inform-done"))
        handle_failure_state = State(HandleFailureBehaviour(datastore=datastore, handle_failure=self.handle_failure, name="Handle-failure"))
        request_cancel_state = State(RequestCancelBehaviour(datastore=datastore, cancel=self.cancel, name="Request-cancel"))
        await_cancellation_result_state = State(AwaitCancellationResultBehaviour(datastore=datastore, name="Await-cancellation-result"))
        #
