...
class Controller(object):
    ...
    def write(self, key, item):
        self.__model.write(key, item)
            
    def read(self, key):
        return self.__model.read(key)
            
    def take(self, key):
        return self.__model.take(key)

    def entities(self):
        return self.__model.keys()

    def register_observer(self, observer):
        self.__model.register_observer(observer)

    def unregister_observer(self, observer):
        try:
            self.__model.unregister_observer(observer)
        except:
            pass
...
#        e.y = e.y - dy
    ...
    def neighbours(self, key, type=None):
        ...
        print "neighbours", key, type, self._model.values()
        return []
        try:
            d = 10    #TODO: use view distance of item
